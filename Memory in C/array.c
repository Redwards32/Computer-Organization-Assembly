...
int bSearch(int *arr, int a, int b, int key) {
    // Binary search function. arr is the array, key is the value to search for, a 
and b are the boundaries of arr to be searched within.
    // You must use pointer notations. i.e. no "[]"
    // Your code goes here:
    int mid = (a + b) / 2;
    int val_of_search = *(arr + mid);
    ...
    if(val_of_search == key){
        return mid;
    }else if(key > val_of_search){
        return bSearch(arr, mid, b, key);
    }else{
        return bSearch(arr, a, mid, key);
    }
}
int main() {
